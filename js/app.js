...
function buildWallSegment(wall) {
  ...
  if (wType === 'door') {
    ...
    const makeLeaf = (hingeS, leafW, dir) => {
      ...
      pivot.rotation.y = rotY - dir * 0.32;
  ...
  openings.forEach(o => {
    ...
    if (o.kind === 'window') {
      ...
      // Glass panel (slightly offset / ajar)
      const gPanel = new THREE.Mesh(new THREE.BoxGeometry(ow * 0.62, DOOR_H - 0.04, 0.04), glassMat());
      const [gx, gz] = ptAt(o.s0 + ow * 0.2);
      gPanel.position.set(gx, DOOR_H / 2, gz); gPanel.rotation.y = rotY;
      ...
      pivL.rotation.y = rotY - 0.32; roomGroup.add(pivL); registerWall(slabL); slabL.userData.fadeNormal = dfn;
      ...
      pivR.rotation.y = rotY + 0.32; roomGroup.add(pivR); registerWall(slabR); slabR.userData.fadeNormal = dfn;
      ...
      lpivot.rotation.y = rotY - 0.35; roomGroup.add(lpivot); registerWall(lslab);
      ...
      // Two panels (each 40% of opening, slid open leaving 20% gap)
      const fpW = ow * 0.40, fpH = DOOR_H - 0.07;
      ...
      const spW = ow * 0.44, spH = DOOR_H - 0.06;
      ...
      // Frosted glass panel (partially open - slid to one side)
      const bgW = ow * 0.60, bgH = DOOR_H - 0.06;
      const [bgx, bgz] = ptAt(o.s0 + bgW / 2 + 0.02);
      ...
      // Handle
      const [bhx, bhz] = ptAt(o.s0 + bgW + 0.015);
      ...
      tpivot.rotation.y = rotY - 0.30; roomGroup.add(tpivot); registerWall(tslab);
      ...
      slab.castShadow = true; pivot.add(slab); pivot.rotation.y = rotY - 0.35;
      roomGroup.add(pivot); registerWall(slab);
...
async function boot() {
  ...
  if (saved && saved.items && saved.items.length > 0) {
    ...
    banner.innerHTML = `<div class="rb-text"><b>前回のレイアウトが見つかりました</b><br><span style="font-size:11px;color:var(--text-secondary)">${saved.items.length} 点の家具 / ${saved.room?.w||'?'}m × ${saved.room?.d||'?'}m</span></div>
      <div class="rb-btns">
        <button class="rb-restore" id="rb-yes">復元する</button>
        <button id="rb-no">新規スタート</button>
      </div>`;
    document.body.appendChild(banner);
    document.getElementById('rb-yes').addEventListener('click', () => {
      applyLayout(saved); banner.remove();
      setStatus('前回のレイアウトを復元しました');
    });
    document.getElementById('rb-no').addEventListener('click', () => {
      applyLayout(PRESETS[0]); banner.remove();
