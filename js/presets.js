...
const PRESETS = [
  // ===== アルミ製造工場 (デフォルト) =====
  {
    ...
    items: [
      ...
      P('walltv',      -13.7,  -8.0,  90, '#2a2a2a'),  // 西壁へ寄せ (距離0.3m→閾値内)
  ...
  },
  {
    ...
    walls: {
      ...
      south: [
        { t: 0.50, w: 2.2, kind: 'auto_door' },
        { t: 0.15, w: 1.2, kind: 'window' },
        { t: 0.85, w: 1.2, kind: 'window' },
      ],
    },
    partitions: [
      // プロジェクトルーム左壁 (x=3.5, z=-6〜-2)
      { x1: 3.5, z1: -6,   x2: 3.5, z2: -2 },
      // プロジェクトルーム南壁 (z=-2, x=3.5〜9) ガラスドア付き
      { x1: 3.5, z1: -2,   x2: 9,   z2: -2,  openings: [{ t: 0.45, w: 1.2, kind: 'glass_door' }] },
      // 集中ブースゾーン左壁 (x=3.5, z=-2〜3) 南端にドア
      { x1: 3.5, z1: -2,   x2: 3.5, z2:  3,  openings: [{ t: 0.92, w: 1.0, kind: 'door' }] },
      // ブース仕切り (z=0.5, x=3.5〜9)
    ...
    items: [
      // ===== フォーカスゾーン (北窓沿い個人デスク) =====
      P('desk',  -7,  -5.6, 0, '#dcd6cc'), P('ochr', -7,  -4.8, 0),
      P('desk',  -5,  -5.6, 0, '#dcd6cc'), P('ochr', -5,  -4.8, 0),
      P('desk',  -3,  -5.6, 0, '#dcd6cc'), P('ochr', -3,  -4.8, 0),
      P('desk',  -1,  -5.6, 0, '#dcd6cc'), P('ochr', -1,  -4.8, 0),
      P('desk',   1,  -5.6, 0, '#dcd6cc'), P('ochr',  1,  -4.8, 0),
      // モニター & デスクランプ
      ...
      P('ochr', -7.1, -2.5, 270), P('ochr', -4.9, -2.5, 90),
      P('dtable', -2.0, -2.0, 0, '#b89878'),
      P('ochr', -2.0, -3.0, 0),   P('ochr', -2.0, -1.0, 180),
      P('ochr', -3.1, -2.0, 270), P('ochr', -0.9, -2.0, 90),
      // 6人コラボテーブル (会議テーブル活用)
      P('conftable', 0.2, -4.2, 0, '#8a7060'),
      P('ochr', -1.6, -4.9, 0), P('ochr',  0.2, -4.9, 0), P('ochr', 2.0, -4.9, 0),
      P('ochr', -1.6, -3.5, 180), P('ochr', 0.2, -3.5, 180), P('ochr', 2.0, -3.5, 180),
      // 立ち話用ラウンドテーブル
      P('roundtable', 1.5, -1.2, 0, '#6a5848'),
      P('ochr', 1.5, -2.1, 0),   P('ochr', 1.5, -0.3, 180),
      P('ochr', 0.6, -1.2, 270), P('ochr', 2.4, -1.2, 90),
      // 西壁ホワイトボード (ブレスト用)
      ...
      // ===== エントランス & ロッカーエリア =====
      P('reception',  0.0, 5.55, 180, '#f0ebe0'),
  ...
  },
  {
    ...
    items: [
      ...
      P('ctrlpanel',  0.7, -3.4, 0, '#3a4250'),  // 北壁近く・北扉外側へ移動
      // 計測作業台
  ...
  },
  {
    ...
    items: [
      ...
      P('ind_furnace', 0.5,  4.5, 0,  '#2a2a2a'),  // pallerackとの重なりを解消
      // 管理コーナー (左仕切り内)
  ...
  },
  {
    ...
    items: [
      ...
      // トイレ (左中) — 西壁窓(t=0.68, z≈1.8)との干渉を避け北側へ
      P('toilet', -5.5, 1.25, 90, '#e8e2d6'), P('handbasin', -5.7, 2.4, 90),
      // 洗面所 (左) — vanityを西壁窓(t=0.42, z≈-0.8)の北側へ移動
      P('vanity', -5.6, -1.7, 90, '#e8e2d6'), P('washer', -3.0, -1.4, 0, '#e8e2d6'),
      // 浴室 (左上)
      ...
      P('closet', 5.7, -3.5, 270, '#f3ece0'), P('zzplant', 2.6, -2.9),  // 東壁に背面・西向き(dbed干渉解消)
      P('dresser', 2.55, -3.6, 90, '#f3ece0'),  // 東向き: 東壁仕切りに近づけて背面チェック通過
    ],
  },
  // ===== 学校 (教室) =====
  {
    ...
    items: [
      ...
      P('desk', 0, -2.8, 0, '#e8e2d6'),   // 教卓 (前面=南/生徒側)
      P('ochr', 0, -2.1, 180),            // 教師椅子: 机の南側・北向き(黒板方向)
  ...
  // ===== ホテル (客室) =====
  {
    ...
    items: [
      ...
      P('pendlamp', -0.5, -1.5),  // ベッド左側(西)に移動 — ベッドフットプリント外
    ],
  },
  // ===== 日本家屋 (和室) =====
  {
