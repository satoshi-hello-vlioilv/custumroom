...
const PRESETS = [
  ...
  },
  {
    ...
    walls: {
      // 客用入口は南(前面)。北壁は厨房の背面壁なので開口なし(窓のみ東西)
      south: [{ t:0.5,  w:1.5, kind:'double_door' }],
  ...
  },
  {
    ...
    walls: {
      // 客用入口(ガラス戸)は南(前面・客席側)。北壁はバックキッチンの背面なので窓のみ
      north: [{ t:0.25, w:1.4, kind:'window' }, { t:0.75, w:1.4, kind:'window' }],
      south: [{ t:0.5, w:1.4, kind:'glass_door' }],
      east:  [{ t:0.35, w:1.2, kind:'window' }, { t:0.7, w:1.2, kind:'window' }],
  ...
  },
  {
    ...
    partitions: [
      // プロジェクトルーム左壁 (x=3.5, z=-6〜-2) — ガラス壁
      { x1: 3.5, z1: -6,   x2: 3.5, z2: -2,  type: 'glass' },
      // プロジェクトルーム南壁 (z=-2, x=3.5〜9) — ガラス壁＋ガラスドア
      { x1: 3.5, z1: -2,   x2: 9,   z2: -2,  type: 'glass', openings: [{ t: 0.45, w: 1.2, kind: 'glass_door' }] },
      // 集中ブースゾーン左壁 (x=3.5, z=-2〜3) — ガラス壁＋出入口
      { x1: 3.5, z1: -2,   x2: 3.5, z2:  3,  type: 'glass', openings: [{ t: 0.92, w: 1.0, kind: 'door' }] },
  ...
  },
  {
    ...
    walls: {
      ...
      west:  [{ t:0.42, w:0.8, kind:'window' }, { t:0.68, w:0.8, kind:'window' }],  // t=0.12窓は撤去(ユニットバス壁面の裏に当たるため)
    },
    partitions: [
  ...
  // ===== 子供部屋 (女の子の部屋) =====
  {
    ...
    items: [
      ...
      P('garland', 0.0, -2.46, 0),                     // 北壁ガーランド (ベッド上)
  ...
  // ===== 学校 (教室) =====
  {
    ...
    items: [
      ...
      // 教卓: 机の前面(+Z/アクセス側=引き出しの反対の膝入れ)を北の教師側へ向ける(rotY=180)。
      // 引き出し面は南=生徒側を向く。教師は机の北側に着座し南(生徒)を向く。
      P('desk', 0, -2.8, 180, '#e8e2d6'),   // 教卓 (アクセス面=北/教師側, 引き出し=南/生徒側)
      P('ochr', 0, -3.35, 0),               // 教師椅子: 机の北側・南向き(生徒に正対)
      // 学習机 5列×4行 (机 rotY=0=アクセス面/本入れ=南 / 生徒椅子 南側・北向きで正対)
  ...
  // ===== ホテル (客室) =====
  {
    ...
    items: [
      ...
      P('pendlamp', -0.75, -1.3),  // ベッド西脇の吊り下げ灯(天井マウント・笠は頭上1.85m) — ベッド footprint 外
    ],
  },
  // ===== 日本家屋 (和室) =====
  {
