...
const PRESETS = [
  // ===== 実物件サンプル 3LDK (図面: 芯々 6,600×11,550mm) =====
  {
    id: 'apt_3ldk_sample', name: '実物件サンプル 3LDK', icon: 'fa-building', cat: '住宅',
    desc: '実在物件の間取り図（芯々6,600×11,550mm・3LDK＋バルコニー）を同縮尺で再現。洋室(1)7.0帖・洋室(2)5.5帖・和室4.5帖・LD12.8帖・キッチン3.6帖。レグザ55E770S／IKEA FJÄLLBO・VALNÄS・UGGLERUM・FÅGELFJÄLLET をメーカー公式寸法で配置。',
    room: { w: 6.5, d: 11.5 }, floorType: 'wood', wallType: 'white',
    // 座標: 画面上(-z)=開放廊下側(北)・画面下(+z)=バルコニー側(南)・左(-x)=西・右(+x)=東。
    //  図面の内法実測 約6.23×11.36m を、0.5mグリッド外形 6.5×11.5m(壁厚0.12→内法6.38×11.38m)に横方向1.024倍で当てはめ。
    //  ※0.5mセルが奇数個のため外形は x∈[-3.0,3.5], z∈[-5.5,6.0] (中心が+0.25ずれる)。以下の座標はその実際の外形に合わせてある。
    walls: {
      south: [   // 上辺 = 開放廊下側の外壁 (玄関ドア・通格子付き窓)
        { t: 0.271, w: 1.5,  kind: 'window' },      // 洋室(1) 窓
        { t: 0.514, w: 0.85, kind: 'door' },        // 玄関ドア
        { t: 0.765, w: 1.25, kind: 'window' },      // 洋室(2) 窓
      ],
      north: [   // 下辺 = バルコニー側サッシ (FIX + 引違い + FIX, 幅計約4.8m)
        { t: 0.203, w: 1.55, kind: 'window', sill: 0 },   // FIX (床まで)
        { t: 0.489, w: 2.1,  kind: 'glass_door' },        // 引違い掃き出し窓 (バルコニー出入口)
        { t: 0.746, w: 1.18, kind: 'window', sill: 0 },   // FIX (床まで)
      ],
      east: [], west: [],
    },
    partitions: [
      // --- 左列: 洋室(1) / ファミリークローク / 押入・物入 / 和室 ---
      { x1: -0.12, z1: -5.5, x2: -0.12, z2: -1.75, openings: [{ t: 0.853, w: 0.8, kind: 'door' }] },    // 洋室(1)東壁(玄関・廊下側)。扉は南端寄り, 室内へ開く
      { x1: -3.0, z1: -1.75, x2: -0.12, z2: -1.75 },                                                     // 洋室(1)南壁 = ファミリークローク北壁
      { x1: -0.12, z1: -1.75, x2: -0.12, z2: -0.51, openings: [{ t: 0.379, w: 0.8, kind: 'door' }] },    // ファミリークローク東壁 (廊下から入る)
      { x1: -3.0, z1: -0.51, x2: -0.12, z2: -0.51 },                                                     // FC南壁 = 押入・物入 北壁
      { x1: -0.77, z1: -0.51, x2: -0.77, z2:  0.38 },                                                     // 押入 | 物入(可動棚)
      { x1: -0.12, z1: -0.51, x2: -0.12, z2:  0.38, openings: [{ t: 0.494, w: 0.6, kind: 'door' }] },    // 物入 東壁 (廊下側の扉)
      { x1: -3.0, z1:  0.38, x2: -0.77, z2:  0.38, openings: [{ t: 0.50,  w: 1.9, kind: 'fusuma' }] },  // 押入の襖 (和室側)
      { x1: -0.77, z1:  0.38, x2: -0.12, z2:  0.38 },                                                     // 物入 南壁
      { x1: -0.12, z1:  0.38, x2: -0.12, z2:  2.79, openings: [{ t: 0.502, w: 0.7, kind: 'fusuma' }] },  // 和室 東壁 (廊下側の引戸)
      { x1: -3.0, z1:  2.79, x2: -0.12, z2:  2.79, openings: [{ t: 0.368, w: 1.75, kind: 'fusuma' }] }, // 和室 南壁 (LD側 3枚引戸)
      // --- 中央列: 玄関・廊下 と 右列との境 (洋室2扉 / トイレ扉 / 洗面室扉 / キッチン扉) ---
      { x1:  0.79, z1: -4.15, x2:  0.79, z2:  3.41, openings: [
          { t: 0.205, w: 0.8,  kind: 'door' },          // 洋室(2) 扉 (室内へ開く)
          { t: 0.481, w: 0.7,  kind: 'toilet_door' },   // トイレ扉 (廊下へ開く)
          { t: 0.586, w: 0.7,  kind: 'door' },          // 洗面室 扉
          { t: 0.765, w: 0.75, kind: 'door' },          // キッチン 扉 (廊下→キッチン)
      ] },
      { x1:  1.22, z1: -5.5, x2:  1.22, z2: -4.15 },                                                     // シューズボックス収納の東壁
      { x1:  0.79, z1: -4.15, x2:  1.22, z2: -4.15 },                                                     // 同 南壁 (洋室2との境)
      // --- 右列: 洋室(2) / システム収納 / トイレ / 浴室(1418) / 洗面室 / キッチン ---
      { x1:  1.77, z1: -2.14, x2:  3.5, z2: -2.14 },                                                     // 洋室(2) 南壁 (浴室側)。x0.54〜1.52 はシステム収納の凹み(洋室2側に開口)
      { x1:  0.79, z1: -1.47, x2:  1.77, z2: -1.47 },                                                     // システム収納 南壁 = トイレ 北壁
      { x1:  1.77, z1: -2.14, x2:  1.77, z2: -0.12 },                                                     // 収納・トイレ | 浴室
      { x1:  0.79, z1: -0.12, x2:  3.5, z2: -0.12, openings: [{ t: 0.535, w: 0.7, kind: 'bath_door' }] }, // トイレ・浴室 南壁 = 洗面室 北壁 (浴室扉)
      { x1:  0.79, z1:  1.18, x2:  3.5, z2:  1.18 },                                                     // 洗面室 南壁 = キッチン 北壁
      { x1:  3.25, z1:  2.76, x2:  3.25, z2:  3.91 },                                                     // キッチンカウンター東端の袖壁 (PS)
      { x1:  3.25, z1:  3.91, x2:  3.5, z2:  3.91 },                                                     // PS 南面
      // --- 柱型 (廊下側の両角, 約0.45×0.5m) ---
      { x1: -2.48, z1: -5.5, x2: -2.48, z2: -4.93 }, { x1: -3.0, z1: -4.93, x2: -2.48, z2: -4.93 },
      { x1:  2.92, z1: -5.5, x2:  2.92, z2: -4.93 }, { x1:  2.92, z1: -4.93, x2:  3.5, z2: -4.93 },
    ],
    floors: [
      { x1: -0.12, z1: -5.5, x2:  1.22, z2: -4.15, type: 'genkan' },   // 玄関土間
      { x1: -3.0, z1:  0.38, x2: -0.12, z2:  2.79, type: 'tatami' },   // 和室 4.5帖
      { x1:  0.79, z1: -1.47, x2:  1.77, z2: -0.12, type: 'tile' },     // トイレ
      { x1:  1.77, z1: -2.14, x2:  3.5, z2: -0.12, type: 'tile' },     // 浴室(1418)
      { x1:  0.79, z1: -0.12, x2:  3.5, z2:  1.18, type: 'tile' },     // 洗面室
      { x1:  0.79, z1:  1.18, x2:  3.5, z2:  3.41, type: 'tile' },     // キッチン
    ],
    items: [
      // ===== 洋室(1) 主寝室 — IKEA FÅGELFJÄLLET 132×207 =====
      // ヘッドボードをFC側の壁(z=-2.00)に付け, 足元を廊下側の窓へ (窓前 約1.5m)。西壁沿いに置き, 東側に約1.5mの通路+乗り降りスペース。
      P('fagelfjallet_bed', -2.28, -2.84, 180, '#5b7f9b'),
      // ===== リビング (西壁 = テレビ壁面) =====
      P('fjallbo_tv',   -2.76,  4.3, 90,  '#5b3d29'),          // テレビ台: 西壁に密着, 東向き
      P('regza55e770s', -2.76,  4.3, 90,  '#1c1c1f', 0.54),    // テレビ: テレビ台の上 (天板高0.54m)
      P('ugglerum_ct',  -1.705, 4.3, 90,  '#6b4a30'),          // コーヒーテーブル: テレビ台前面から0.55m
      P('valnas_sofa2', -0.56,  4.3, 270, '#d8c7a6'),          // ソファ: 西向き(テレビ正対), テーブルとの間0.35m, サッシまで0.6m
      // ===== 建物付帯設備 (図面記載のもの) =====
      P('kitchen',    2.05,   3.085, 180, '#e8e2d6'),   // 対面キッチン L=2.4m (作業側=北, LD側にカウンター)
      P('barcounter', 2.05,   3.66,  0,   '#8a6a4a'),   // LD側カウンター
      P('bathset',    2.61,  -1.28,  0,   '#e8e2d6'),   // ユニットバス 1418 (扉側=洗面室)
      P('toilet',     1.28,  -1.06,  0,   '#e8e2d6'),   // トイレ (背面=北壁, 扉は西壁)
      P('vanity',     3.19,   0.53,  270, '#e8e2d6'),   // 洗面化粧台 L=1.2 (東壁, 西向き)
      P('shoebox',    0.985, -4.8,  270, '#5b5048'),   // シューズボックス (玄関脇の収納内, 玄関向き)
    ],
    notes: [
      '【縮尺】図面の芯々 6,600×11,550mm を 0.5mグリッドの外形 6.5×11.5m（壁厚0.12m→内法 約6.38×11.38m）に収め、内部の壁・開口は図面の内法実測（約6.23×11.36m）を横方向1.024倍で当てはめています（誤差は数cm）。',
      '【寝室（洋室1）】FÅGELFJÄLLET（幅132×長さ207cm）はヘッドボードをファミリークローク側の壁に付け、足元を廊下側の窓に向けて窓前に約1.5mを確保。西壁沿いに寄せ、東側に約1.5mの通路＝乗り降りスペース。扉（幅0.8m・室内側に開く）の開閉範囲は南東角で、ベッドとは0.7m離れています。',
      '【リビング】テレビ台 FJÄLLBO（150cm）を西壁に付け、レグザ 55E770S（幅122.6cm）を載せています（台の奥行36cm ≧ スタンド奥行28.7cm）。テレビ台〜UGGLERUM（130×65cm）の間は0.55m（人が通れる幅）、テーブル〜VALNÄS（206cm）の膝前は0.35m。視聴距離は画面〜座面前端 約1.7m（目の位置で約2.2m）。',
      '【窓・動線】バルコニーの引違い窓（幅2.1m）の前は幅1.3mで常時空け、ソファの端から窓まで0.6m。和室の3枚引戸（1.75m）と押入の襖の前にも家具を置いていません。廊下→LD→キッチンカウンター前へは、ソファ背面とカウンター端の間 約0.85m を通ります。',
      '【未配置】対面キッチン（L=2.4m）前のダイニングゾーン（約2.6×2.0m）、洋室(2)、和室は空けてあります。家具はドラッグで移動、R キーまたは右パネルの回転ボタンで回転、矢印キーでスナップ幅ずつ微調整できます（右下のスナップ幅で 0.05m まで）。',
      '【寸法の出典】各家具を選択すると右パネル「実寸・出典」に採用寸法（mm）とメーカー公式ページへのリンクを表示します。「周囲との間隔」には上下左右の最寄りの壁・家具までの距離が出ます。',
    ],
  },
  // ===== アルミ製造工場 (デフォルト) =====
  {
    ...
    items: [
      ...
      P('inj_molder', -12.0, -2.3, 90),   // 全長5.1m: 制御室壁(z=-5)と原材料エリア(z=+2)の間に収める
  ...
  },
  {
    ...
    items: [
      ...
      P('stable',  7.3, -0.8, 0),
      P('desklamp', 7.3, -0.8, 0, undefined, 0.45),
  ...
  },
  {
    ...
    items: [
      ...
      P('glassware',  -2.9, 1.2, 180),  // labbench(W1800)上: -3.5±0.9 の範囲内
      // デスク作業
  ...
  },
  {
    ...
    items: [
      ...
      P('toolcab',    0.15, 1.6, 0, '#c0392b'),  // 作業台(x=-1.0±0.75)の東隣に並べる
      P('toolcab',    0.95, 1.6, 0, '#c0392b'),
  ...
  // ===== 子供部屋 (女の子の部屋) =====
  {
    ...
    items: [
      ...
      P('kidsdesk', 2.15, -0.6, 270, '#e6cfa3'),
      ...
      // コーナーの玩具
      P('toybox', -2.0, 1.95, 0, '#e6cfa3'),
      P('dollhouse', 1.9, 1.9, 180, '#e6cfa3'),
