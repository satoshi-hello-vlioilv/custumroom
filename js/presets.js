...
const PRESETS = [
  ...
  },
  {
    ...
    items: [
      ...
      P('whiteboard', 6.92, -3.8, 270),  // 東壁に背面→室内(西)向き (原点=壁内面で面一)
  ...
  },
  {
    ...
    items: [
      ...
      P('whiteboard', -3.92, 0.0, 90),                          // 西壁 (窓t=0.3/0.7を避け中央へ。原点=壁内面で面一)
  ...
  },
  {
    ...
    partitions: [
      ...
      // プロジェクトルーム南壁 (z=-2, x=3.5〜9) — ガラス壁＋ガラスドア。会議室⇔ブースA を仕切る実壁で,
      //  両室を分ける役割があるため扉は有意(壁を回り込めない)
      { x1: 3.5, z1: -2,   x2: 9,   z2: -2,  type: 'glass', openings: [{ t: 0.45, w: 1.2, kind: 'glass_door' }] },
      // 集中ブースゾーン左壁 (x=3.5, z=-2〜3) — ガラス壁。ブースA(z=-2〜0.5)は四方を壁で囲うため専用扉(z≈-0.5)が要る。
      //  ブースB(z=0.5〜3)は北側(z=3)がラウンジ/カフェへ開放=この壁を回り込めるので, 西側の扉は意味がなく置かない
      { x1: 3.5, z1: -2,   x2: 3.5, z2:  3,  type: 'glass', openings: [{ t: 0.3, w: 1.0, kind: 'door' }] },
    ...
    items: [
      // ===== フリーアドレス島 (シームレス両面ベンチデスク) =====
      // 4ユニットを X 端で突き合わせた一体の島。1人当たり 1.6m×0.75m と広め。
      // 南北両側に着席 (南席=rotY0で島(北)を向く / 北席=rotY180で島(南)を向く)。
      P('benchdesk2', -5.4, -4.3, 0, '#dcd6cc'),
      P('benchdesk2', -3.8, -4.3, 0, '#dcd6cc'),
      P('benchdesk2', -2.2, -4.3, 0, '#dcd6cc'),
      P('benchdesk2', -0.6, -4.3, 0, '#dcd6cc'),
      // 南側着席 (4席・北向き)
      P('ochr', -5.4, -5.4, 0), P('ochr', -3.8, -5.4, 0), P('ochr', -2.2, -5.4, 0), P('ochr', -0.6, -5.4, 0),
      // 北側着席 (4席・南向き)
      P('ochr', -5.4, -3.2, 180), P('ochr', -3.8, -3.2, 180), P('ochr', -2.2, -3.2, 180), P('ochr', -0.6, -3.2, 180),
      // モニター (中央スパインに背中合わせ。南席用=南向き180 / 北席用=北向き0)
      P('monitor', -5.4, -4.62, 180, undefined, 0.73), P('monitor', -3.8, -4.62, 180, undefined, 0.73),
      P('monitor', -2.2, -4.62, 180, undefined, 0.73), P('monitor', -0.6, -4.62, 180, undefined, 0.73),
      P('monitor', -5.4, -3.98, 0, undefined, 0.73), P('monitor', -3.8, -3.98, 0, undefined, 0.73),
      P('monitor', -2.2, -3.98, 0, undefined, 0.73), P('monitor', -0.6, -3.98, 0, undefined, 0.73),
      // 島端のアクセント植物
      P('sansevieria', -6.7, -4.3), P('sansevieria', 0.7, -4.3),

      // ===== コラボレーションゾーン (フリーアドレス島の北側。島を深くした分だけ北へ寄せて確保) =====
      // 4人チームテーブル ×2 (東西の椅子はテーブル中心に正対: 西側=東向き90 / 東側=西向き270)
      P('dtable', -6.0, -1.2, 0, '#b89878'),
      P('ochr', -6.0, -2.2, 0),  P('ochr', -6.0, -0.2, 180),
      P('ochr', -7.1, -1.2, 90), P('ochr', -4.9, -1.2, 270),
      P('dtable', -2.0, -0.7, 0, '#b89878'),
      P('ochr', -2.0, -1.7, 0),  P('ochr', -2.0, 0.3, 180),
      P('ochr', -3.1, -0.7, 90), P('ochr', -0.9, -0.7, 270),
      // 立ち話用ラウンドテーブル
      P('roundtable', 1.5, 0.1, 0, '#6a5848'),
      P('ochr', 1.5, -0.8, 0),  P('ochr', 1.5, 1.0, 180),
      P('ochr', 0.6, 0.1, 90), P('ochr', 2.4, 0.1, 270),
      // 西壁ホワイトボード (ブレスト用。原点=壁内面で面一に壁掛け)
      P('whiteboard', -8.92, -3.5, 90, '#f8f5f0'),
      P('whiteboard', -8.92, -0.5, 90, '#f8f5f0'),
      // コラボゾーン装飾
      P('monstera',  -8.5, -1.5),
      P('zzplant',    2.8, -1.5),
      ...
      P('whiteboard', 7.5, -5.92, 0, '#f8f5f0'),   // 南壁 (窓を避けて右寄り。原点=壁内面で面一)
  ...
  },
  {
    ...
    items: [
      ...
      P('whiteboard', -5.42, 0.0, 90),  // 西壁面に面一で壁掛け (原点=壁内面)
      // 防振定盤エリア (右区画)
