...
const PRESETS = [
  ...
  },
  {
    ...
    walls: {
      ...
      south: [{ t:0.22, w:1.0, kind:'door' }, { t:0.75, w:1.2, kind:'window' }],   // 玄関扉: ユニットバス(西)とキッチンの間へ(バス背後/前を避ける)
  ...
  // ===== 学校 (教室) =====
  {
    ...
    partitions: [],
    items: [
      // 前方 (南) — 黒板・教卓 (黒板は南壁=無開口に背を付け, 前面+Z=室内/生徒側を向く rotY=0)
  ...
  // ===== ホテル (客室) =====
  {
    ...
    walls: {
      north: [],
      south: [{ t: 0.31, w: 1.0, kind: 'door' }],   // 客室入口(ユニットバス東隣の入口通路, バス背後を避ける)
