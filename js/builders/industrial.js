...
function buildBandedAlumCoil({ color='#c8c8cc', w=1.0, d=0.8, h=0.8 } = {}) {
  const g = new THREE.Group();
  const alum  = mat(color, 0.2, 0.85, { env: 0.9 });
  const core_m= mat('#888', 0.4, 0.5);
  const bandM = mat('#6a7080', 0.35, 0.55);
  const claspM= mat('#8a9098', 0.3, 0.65);
  const coilR = Math.min(w, h) / 2, coreR = coilR * 0.28;
  // Main coil body
  const coil = new THREE.Mesh(new THREE.CylinderGeometry(coilR, coilR, d, 32), alum);
  ...
  // Inner core
  const core = new THREE.Mesh(new THREE.CylinderGeometry(coreR, coreR, d + 0.02, 16), core_m);
  core.rotation.x = Math.PI/2; core.position.set(0, coilR, 0); g.add(core);
  // End face rings
  [-(d/2), d/2].forEach(fz => {
    for (let r = coreR + 0.03; r < coilR - 0.01; r += 0.06) {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(r, 0.012, 4, 28), alum);
      ring.rotation.x = Math.PI/2; ring.position.set(0, coilR, fz); g.add(ring);
    }
    const face = new THREE.Mesh(new THREE.CircleGeometry(coilR, 32), alum);
    face.rotation.x = -Math.PI/2; face.position.set(0, coilR, fz + (fz < 0 ? -0.005 : 0.005));
...
function buildPackagedAlumCoil({ color='#b4a478', w=1.0, d=0.8, h=0.8 } = {}) {
  ...
  // Eye (exposed center hole) visible on both faces
  [-(d/2 + 0.02), (d/2 + 0.02)].forEach(fz => {
    const eye = new THREE.Mesh(new THREE.RingGeometry(coreR, coilR * 0.42, 16), alumM);
    eye.rotation.x = -Math.PI/2; eye.position.set(0, coilR, fz);
...
function buildExportAlumCoil({ color='#c0a870', w=1.2, d=1.2, h=1.0 } = {}) {
  ...
  // Coil eye (aluminum ring visible on top face)
  const eye = new THREE.Mesh(new THREE.RingGeometry(coilR*0.28, coilR*0.58, 20), alumM);
  eye.rotation.x = -Math.PI/2; eye.position.set(0, 0.11 + ph + 0.002, 0); g.add(eye);
...
function buildAlumCoilSide({ color='#c8c8cc', w=0.8, d=1.0, h=0.8 } = {}) {
  ...
  const bandM  = mat('#6a7080', 0.35, 0.55);
  // When on side, axis is horizontal (along Z in original, now along X)
  // coilR = h/2 (visible height from front = diameter)
  const coilR = h / 2, coreR = coilR * 0.28, coilDepth = d;
  // Coil body (cylinder axis along X, lying on curved surface)
  const coil = new THREE.Mesh(new THREE.CylinderGeometry(coilR, coilR, coilDepth, 32), alum);
  coil.rotation.z = Math.PI/2; coil.position.set(0, coilR, 0);
  coil.castShadow = true; coil.receiveShadow = true; coil.userData.colorable = true; g.add(coil);
  // Inner core hole
  const core = new THREE.Mesh(new THREE.CylinderGeometry(coreR, coreR, coilDepth + 0.02, 16), core_m);
  core.rotation.z = Math.PI/2; core.position.set(0, coilR, 0); g.add(core);
  // End face rings (layered sheet look, both sides)
  [-(coilDepth/2), coilDepth/2].forEach(fx => {
    for (let r = coreR + 0.03; r < coilR - 0.01; r += 0.06) {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(r, 0.012, 4, 28), alum);
      ring.rotation.z = Math.PI/2; ring.position.set(fx, coilR, 0); g.add(ring);
    }
    const face = new THREE.Mesh(new THREE.CircleGeometry(coilR, 32), alum);
    face.rotation.y = Math.PI/2; face.position.set(fx + (fx < 0 ? -0.005 : 0.005), coilR, 0);
    if (fx < 0) face.rotation.y = -Math.PI/2; g.add(face);
  });
  // Steel banding bands (2 bands perpendicular to axis → rings in YZ plane)
  [-0.18, 0.18].map(t => t * coilDepth).forEach(bx => {
    const band = new THREE.Mesh(new THREE.TorusGeometry(coilR + 0.016, 0.018, 6, 32), bandM);
    band.rotation.z = Math.PI/2; band.position.set(bx, coilR, 0); g.add(band);
